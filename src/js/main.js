...
function init() {
  ...
  document.addEventListener('keypress', (ev) => {
    ...
    if (timestamp && !timeTaken && !loading && choices.length === battleNo - 1) {
      switch(ev.key) {
        case 's': case '2':                   saveProgress('Progress'); break;
        case 'j':                             pick('left'); break;
        case 'l':                             pick('right'); break;
        case 'q':                             pick('tie'); break;
        case 'k': case '1':                   undo(); break;
        default: break;
      }
    }
...
function generateImage() {
  ...
  html2canvas(document.querySelector('.results'), {
	  width: document.querySelector('.results').scrollWidth,
	  height: document.querySelector('.results').scrollHeight,
	  scrollX: 0,
	  scale: 2
}).then(canvas => {
